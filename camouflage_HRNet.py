...
def main(args):
    ...
    assert (
        h < h_origin
    ), "mask height must be smaller than bg height, and lower mask_scale parameter!!"
    assert (
        w < w_origin
    ), "mask width must be smaller than bg width, and lower mask_scale parameter!!"
    ...
    mat_dilated = fore * np.expand_dims(mask_crop / 255, axis=-1) + origin[
        y1:y2, x1:x2
    ] * np.expand_dims((mask_dilated - mask_crop) / 255, axis=-1)
    bg = origin.copy()
    bg[y1:y2, x1:x2] = fore * np.expand_dims(mask_crop / 255, axis=-1) + origin[
        y1:y2, x1:x2
    ] * np.expand_dims(1 - mask_crop / 255, axis=-1)
    ...
    for u, layer in enumerate(attention_layers):
        target_feature = (
            target_features[layer].detach().cpu().numpy()
    ...
    foreground_attention = (
        torch.from_numpy(all_attention.astype(np.float32))
        .clone()
        .to(device)
        .unsqueeze(0)
        .unsqueeze(0)
    ...
    size = len(idx[0])
    mask_f = (
        torch.from_numpy(mask_f.astype(np.float32))
        .clone()
        .to(device)
        .unsqueeze(0)
        .unsqueeze(0)
    ...
    mask_norm = mask_crop / 255.0
    mask_norm_torch = (
        torch.from_numpy((mask_norm).astype(np.float32))
        .unsqueeze(0)
        .unsqueeze(0)
        .to(device)
    )
    ...
    while epoch <= steps:
        ...
        target_features = get_features(
            target, VGG
        )  # extract output image's all feature maps
        ...
        content_loss = (
            torch.sum(
                (target_features_content["conv4_2"] - foreground_features["conv4_2"])
                ** 2
            )
            / 2
        )
        ...
        for layer in style_weights:
            ...
            layer_style_loss = (
                style_weights[layer]
                * torch.sum((target_gram_matrix - style_gram_matrix) ** 2)
                / ((2 * c * w * h) ** 2)
            )
        ...
        tv_h_mask = (
            tv_h[:, :, 0][idxes_h] + tv_h[:, :, 1][idxes_h] + tv_h[:, :, 2][idxes_h]
        )
        tv_w_mask = (
            tv_w[:, :, 0][idxes_w] + tv_w[:, :, 2][idxes_w] + tv_w[:, :, 2][idxes_w]
        )
        tv_loss = torch.from_numpy(
            (np.array(np.mean(np.concatenate([tv_h_mask, tv_w_mask]))))
        ).to(device)
        ...
        if epoch % show_every == 0:
            ...
            canvas[y1:y2, x1:x2] = fore_gen * np.expand_dims(
                mask_norm, axis=-1
            ) + origin[y1:y2, x1:x2] * np.expand_dims(1.0 - mask_norm, axis=-1)
            ...
            cv2.imshow(
                "now camouflage...",
                cv2.resize(
                    canvas, (w_show // args.show_comp, h_show // args.show_comp)
                ),
            )
    ...
    canvas = origin.copy()
    fore_gen = im_convert(target) * 255.0
    canvas[y1:y2, x1:x2] = fore_gen * np.expand_dims(mask_norm, axis=-1) + origin[
        y1:y2, x1:x2
    ] * np.expand_dims(1.0 - mask_norm, axis=-1)
    canvas = canvas.astype(np.uint8)
    canvas = cv2.cvtColor(canvas, cv2.COLOR_RGB2BGR)
